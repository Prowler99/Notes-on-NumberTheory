...
\begin{enumerate}
    ...
    \item [\myit] If $F$ is nonarchimedean and ramified, then there exists a minimal integer $N$ s.t. $\chi(1 + \varpi^N\O_F^\times) = \{1\}$, whence $\chi$ factors through the finite group $\O_F^\times/(1 + \varpi^N\O_F^\times)$, and thus \[\chi(x) = |x|^s\chi_0(x),\] where $\chi_0$ is a character of $\O_F^\times/(1 + \varpi^N\O_F^\times)$.
...
\begin{enumerate}
    \item [$\bullet$] If $p = \ell$, then the isomorphism $\Z_\ell^\times/(1+\ell^e\Z_\ell)\simeq (\Z/\ell^e\Z)^\times$ enables us to lift $\chi^{-1}$ (note the `$-1$'!) to a character $\chi_\ell$ on $\Q_\ell^\times$ that is trivial on $\ell^\Z$ and $1+\ell^e\Z_\ell$.
...
\begin{remark}
    The character $\tilde{\chi}$ is
    (the inverse of?)
    \[\idele{\Q}/\Q^\times\to\idele{\Q}/\Q^\times\R_{>0}\simeq\hat{\Z}^\times\to (\Z/N\Z)^\times \stackrel{\chi}{\to}\C^\times.\]
    Conversely, every (unitary) Hecke character factors through $\hat{\Z}^\times\to\C^\times$, and hence it is of finite order iff it comes from a Dirichlet character.
\end{remark}
