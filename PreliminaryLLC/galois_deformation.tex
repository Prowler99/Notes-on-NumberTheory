...
\end{itemize}
A \textbf{framed deformation} of $(V, \beta_k)$
...
\begin{remark}
    ...
    then a framed deformation $(V_A, \iota_A, \beta_A)$ is a representation $\rho_A : G\to\GL_d(A)$ lifting $\bar{\rho}$, namely the map $G\stackrel{\rho_A}{\to}\GL_d(A)\to\GL_d(k)$ is exactly $\bar{\rho}$,
    ...
    if they are, as representations, conjugate by some element in $\ker\left( \GL_d(A)\to\GL_d(k) \right)$.
\end{remark}
...
\begin{proof}
    ...
    So for $A = \varprojlim_{i} A_i$ with $A_i$ Artinian quotients, \begin{align*}
...
Let $(W^\oblong, \iota^\oblong, \beta^\oblong)$ be a framed deformation given by $\rho^\oblong : G\to \GL_d(R_V^\oblong)$.
...
there is an $R_V[G]$-module $W'$ such that $W\otimes_{R_V} R_V^\oblong\simeq W^\oblong$ with an isomophism
\[\iota : W\otimes_{R_V} k \simeq W\otimes_{R_V}R_V^\oblong\otimes_{R_V^\oblong} k\simeq W^\oblong\otimes_{R_V^\oblong} k\stackrel{\iota^\oblong}{\simeq} V\]
\begin{lemma}
    $(W, \iota)$ is a universal deformation of the absolutely irreducible $k[G]$-module $V$.
\end{lemma}
\begin{proof}
