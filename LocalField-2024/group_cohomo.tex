...
Let $G$ be a group. A \textbf{$G$-module} is an abelian group together with an additive \textit{left} $G$-action. Hence the category of $G$-modules is isomorphic to the category of $\Z[G]$-modules.
\begin{example}
We list some important constructions of $G$-modules here.
\begin{enumerate}[(a)]
    \item The \textbf{trivial $G$-module} is $\Z$ with the trivial $G$-action.
    \item The group ring $\Z[G]$ is a $G$-module with $G$ acting by left-multiplication.
    \item Direct sum and product. Both direct sums and products for $G$-modules as abelian groups can be lifted to $G$-modules, by giving $G$-action diagonally.
    \item Tensor products. For $M, N\in \Mod_G$, define $M\otimes N\in \Mod_G$ to be $M\otimes_\Z N$ with the diagonal $G$-action \[g(x\otimes y) := gx\otimes gy,\quad x\in M, y\in N.\]
    \item Hom module. For $M, N\in\Mod_G$, define $\hom(M, N)\in \Mod_G$ to be $\hom_{\Z}(M, N)$ with the $G$ acting ``conjugately'': \[(gf)(x) := g f(g^{-1}x),\quad f\in\Hom_\Z(M, N), x\in M.\]
    \begin{itemize}
        \item We have \[\Hom_G(M, N) = \Hom(M, N)^G\] as $G$-modules.
        \item The adjoint $L\otimes_\Z (-) \dashv \Hom_\Z(L, -)$ in $\Abel$ holds in $\Mod_G$, i.e,
\[\begin{tikzcd}
...
\end{tikzcd}\]
    ...
    \end{itemize}
    \item Induced module. Let $H < G$ be a subgroup, $N$ a $H$-module. Then $\Ind_H^G N$ is the abelian group of $H$-invariant functions $G\to N$, i.e., \[\Ind_H^G N := \{\varphi : G \to N\mid \varphi(hg) = h\varphi(g),\;\forall h\in H, g\in G\}\simeq\Hom_H(\Z[G], N).\]
    ...
    \begin{itemize}
        \item $\Ind_H^G$ is right adjoint to the forgetful functor $\Mod_G\to\Mod_H$. The isomorphism is given by
        \[\begin{tikzcd}
        ...
        \item For any abelian group, we define \[\Ind^G M := \Ind_{\{1\}}^G M =\{\varphi : G\to M\}.\] An \textbf{induced module} is a $G$-module of the form $\Ind^GM$ for some abelian group $M$.
...
We define the \textbf{ $r$-th cohomology groups of $G$ with coefficients in $M$} to be the value of the $r$-th right derived functor of the left-exact functor \[(-)^G\simeq \Hom_G(\Z, -) : \Mod_G\to\Abel\] at $M$.
