...
\begin{lemma}\label{torsion submodule of an ideal}
    ...
    % Then the $A/\mathfrak{a}$-module $\phi[\mathfrak{a}]$ is free of rank $r$.
...
% extending to an embedding $\phi : F_\infty\to L^\perf((\tau^{-1}))$.
...
% \[\exists\sigma\in\aut_k(\F_\infty),\quad \mu_\phi = \sigma\circ\varepsilon.\]
...
% the latter is guanranteed as all Drinfeld modules of rank $r$ over $\C_\infty$ are given by lattices of rank $r$ in $\C_\infty$
(give reference!)
...
% = \varprojlim_e \Hom_A(A/\lambda^e, \phi[\lambda^e]),
\end{align*}
...
\begin{lemma}\label{lem: to define infty-adic representation}
    There exists some series $u\in F^\sep\llbracket\tau^{-1}\rrbracket^\times$, such that \[u^{-1}\phi(F_\infty)u\subset \bar{k}\llpar\tau^{-1}\rrpar.\footnotemark\]
    \footnotetext{Recall that the Hayes module $\phi : A\to H_A^+[\tau]$ extends to an injective homomorphism $\phi : F_\infty\to \left( H_A^+ \right)^\perf\llpar\tau^{-1}\rrpar$.
    }
...
% We will denote the class of $\alpha\in \A_F^\times$ in $C_F$ by $[\alpha]$.
...
% Let $S_U$ be the set of places consisting of $\infty$ and those
% $\mathfrak{p}$ for which there exists some idele $\alpha\in\O_\mathfrak{p}^\times\hookrightarrow\footnotemark \A_F^\times$ whose class in $C_F$ is not in $U$.
